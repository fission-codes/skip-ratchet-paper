...
\begin{document}
    ...
    By analogy, multiple values can placed in a tuple and iteratively hashed to represent separate digits in a KDF.
    ...
    \begin{figure}[h]
       	...
       	\caption{Componentized Positional KDF}
    \end{figure}
	...
	\begin{figure}[h]
		...
		\caption{OKM for a Positional Ratchet}
	\end{figure}
    
    \section{Bounding}
    
    Unlike number systems for arithmetic, the spiral ratchet has a fixed number of ``digits'' in its internal state. This limits the jumps to fixed intervals, with a maximum jump interval.  It would be possible to deterministically generate increasingly large digits as needed, but (as we will see later) this would leak data by revealing the range that the count is currently in.
	...
	\begin{figure}[h]
		...
		$\xymatrix{
		    \left[ { \begin{array}{cc} L \\ M  \\ S \end{array} } \right]
				\ar[r]^{+1}
				\ar@/_3pc/[rr]_{skip_{M}}
				\ar@/^7pc/[rrrr]^{skip_{L}}
    	 & \left[ { \begin{array}{cc} L \\ M  \\ h(S) \end{array} } \right]
	      		\ar[r]^{+1}
	      		\ar@/^4.75pc/[rrr]^{skip_{L}}
	     & \left[ { \begin{array}{cc} L \\ h(M)  \\ S' \end{array} } \right]
	      		\ar[r]^{+1}
	      		\ar@/_3pc/[rr]_{skip_{M}}
	      		\ar@/^3pc/[rr]^{skip_{L}}
    	 & \left[ { \begin{array}{cc} L \\ h(M)  \\ h(S') \end{array} } \right]
	      		\ar[r]^{+1}
	     & \left[ { \begin{array}{cc} h(L) \\ M'  \\ S'' \end{array} } \right]
		}$

		\caption{Simplified Multipart Ratchet}
	\end{figure}

	Each digit is given a label and a maximum value for all but the largest, which acts as a spine. This treats the largest element as analogous to a unary digit, and the remaining elements are bounded by their numerical base. The unary digit hides information about the range of numbers that the current internal state represents, which is called an ``epoch''.
	
	While it is easier to reason about a consistent base, there are cases where the varied skip intervals per level is useful. This paper will only consider the case where the base is consistent across all positional digits.
	...
	\subsection{Spiral Property}
	
	The spiral ratchet does not make for a compelling basis for arithmetic.
	
	The each recursive zeroing is represented with a different value, both between digits and per-digit. This is roughly analogous to moving in a spiral, where one component returns to its initial value, but the other is always moving outwards. In short: a spiral is predictable, but never crosses itself.
	
	\label{spiral-property}
	...
	\begin{figure}[h]
		...
		\caption{A Spiral Never Crosses Itself}
	\end{figure}
    
    
    \section{Acknowledgements}
    
    The spiral ratchet algorithm was developed at \href{https://fission.codes}{Fission Codes} to support work on the \href{https://github.com/fission-suite/webnative}{Webnative File System (WNFS)}. Many thanks to the Fission Codes team for providing space to do this work.
    
    Thanks to \href{https://github.com/matheus23/}{Philipp Krüger} at Fission Codes for  feedback on the design, for doing the bulk of the \href{https://github.com/fission-suite/webnative/blob/e1eed4e750c668a6f54d9be701dc17e286b9eff4/src/fs/data/private/spiralratchet.ts}{TypeScript implementation}, and for his patience as various parts of the algorithm were tweaked.
    
    Thanks to \href{https://github.com/b5/}{Brendan O'Brien} at \href{https://qri.io/}{Qri} for the \href{https://github.com/qri-io/wnfs-go/commit/62915924c866f7106aecacccd14e57bd1b62850f}{Go implementation}, and for many discussions on the best way to explain the concept.
    
    Thanks to and \href{https://github.com/dholms}{Daniel Holmgren} at \textbf{NOT YET PUBLIC} for his feedback on early versions of this algorithm.
